...
export async function GET(
  ...
  try {
    const startUrl = new URL(`/auth/customer/${config.id}`, MEDUSA_BACKEND_URL)
    ...
    if (!medusaResponse.ok) {
      throw new Error(
        `Medusa rejected ${config.id} OAuth start (${medusaResponse.status})`
      )
    }
    ...
    if (!location || typeof location !== "string") {
      throw new Error(
        `Medusa did not return a redirect location for ${config.id} OAuth`
      )
    }
    ...
    if (state) {
      ...
      cookieStore.set(getStateCookieName(config.id, state), encodeURIComponent(returnTo), {
        httpOnly: true,
        sameSite: "lax",
    ...
    const message =
      error instanceof Error
        ? error.message
        : `Failed to start ${config.id} sign-in`
