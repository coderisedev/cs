...
export async function generateStaticParams() {
  const firstPage = await listPosts({ page: 1, pageSize: 50, category: "header" })
  ...
  for (let page = 2; page <= firstPage.pagination.pageCount; page += 1) {
    const pageResult = await listPosts({ page, pageSize: 50, category: "header" })
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string; countryCode: string }> }): Promise<Metadata> {
  ...
  if (!post) {
    return { title: "Announcement", description: "Announcements from Cockpit Simulator" }
  }

  const canonicalUrl = `${BASE_URL}/${countryCode || DEFAULT_COUNTRY}/announcement/${slug}`
...
function generateAnnouncementJsonLd(post: BlogPost, countryCode: string) {
  const url = `${BASE_URL}/${countryCode || DEFAULT_COUNTRY}/announcement/${post.slug}`
  ...
  return {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 2,
        name: "Announcement",
        item: `${BASE_URL}/${countryCode || DEFAULT_COUNTRY}/announcement`,
      },
      {
        "@type": "ListItem",
        position: 3,
        name: post.title,
        item: `${BASE_URL}/${countryCode || DEFAULT_COUNTRY}/announcement/${post.slug}`,
      },
    ],
  }
}
...
export default async function AnnouncementPage({ params }: AnnouncementPageProps) {
  ...
  const announcementJsonLd = generateAnnouncementJsonLd(post, countryCode)
    ...
    <>
      ...
      <article className="container py-16 space-y-8 max-w-3xl">
        <div className="space-y-3">
          <p className="text-xs uppercase tracking-widest text-primary-500">Announcement</p>
