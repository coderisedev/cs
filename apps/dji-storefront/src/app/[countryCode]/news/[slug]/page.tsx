...
export async function generateStaticParams() {
  const firstPage = await listPosts({ page: 1, pageSize: 50, category: "News" })
  ...
  for (let page = 2; page <= firstPage.pagination.pageCount; page += 1) {
    const pageResult = await listPosts({ page, pageSize: 50, category: "News" })
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string; countryCode: string }> }): Promise<Metadata> {
  ...
  if (!post) {
    return { title: "News", description: "Latest news from Cockpit Simulator" }
  }

  const canonicalUrl = `${BASE_URL}/${countryCode || DEFAULT_COUNTRY}/news/${slug}`
...
function generateNewsPostJsonLd(post: BlogPost, countryCode: string) {
  const url = `${BASE_URL}/${countryCode || DEFAULT_COUNTRY}/news/${post.slug}`
  ...
  return {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 2,
        name: "News",
        item: `${BASE_URL}/${countryCode || DEFAULT_COUNTRY}/news`,
      },
      {
        "@type": "ListItem",
        position: 3,
        name: post.title,
        item: `${BASE_URL}/${countryCode || DEFAULT_COUNTRY}/news/${post.slug}`,
      },
    ],
  }
}
...
export default async function NewsPostPage({ params }: NewsPostPageProps) {
  ...
  const newsPostJsonLd = generateNewsPostJsonLd(post, countryCode)
  ...
  return (
    <>
      <script
        ...
        dangerouslySetInnerHTML={{ __html: JSON.stringify(newsPostJsonLd) }}
      />
      <script
        type="application/ld+json"
      ...
      <article className="container py-16 space-y-8 max-w-3xl">
        <div className="space-y-3">
          <p className="text-xs uppercase tracking-widest text-primary-500">News</p>
