...
related_docs:
  ...
  - docs/plan/gce-medusa-strapi-r2-deployment.md
---
...
2. **Cookie Policy Override**  
   ...
   - For the Cloudflare tunnel, `deploy/gce/.env.prod` sets `ADMIN_COOKIE_SECURE=false`, allowing Strapi to emit cookies even though the immediate connection is HTTP.
3. **Secret Refresh & Redeploy**  
   - Rotated all placeholder secrets in `deploy/gce/.env.prod` (JWTs, salts, webhook, etc.) and rebuilt both Medusa/Strapi production images to ensure consistent config.
4. **Config propagation**  
   - Rebuilt `cs-strapi:prod`, ran `docker compose -p cs-prod --env-file deploy/gce/.env.prod -f deploy/gce/prod/docker-compose.yml up -d`, verified logs show the admin URL and no further cookie errors.
...
3. **Secrets hygiene**: Avoid shipping placeholder secrets in `.env.prod`. Automate generation (e.g., `secrets.token_urlsafe`) before first deploy.
