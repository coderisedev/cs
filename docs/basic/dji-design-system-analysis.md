...
- 桌面段落间距建议使用 `py-20`，平板 `py-16`，移动 `py-12`，保持纵向节奏统一。
- 英雄与覆盖卡片使用全局 tokens：`var(--gradient-hero)`（暗色遮罩）与 `var(--gradient-hero-accent)`（品牌渐变），避免手写 `rgba` / `#000` 叠层，便于主题切换。
- CTA 区块在移动端保持 `px-6 py-3` 以内，确保触控可达性。
